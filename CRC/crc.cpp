...
#define CRC8_MSB_CHECK (0x80)
const static uint8 crc8_tab[256] = {
...
const static uint32 crc32_tab[256] = {
...
#define CRC32_MPEG_GENERATOR (0x04C11DB7)
#define CRC32_MSB_CHECK      (0x80000000)

void crc32_table_maker()
{
    uint32 crc32_tab[256] = { 0 };
    uint32 var = 0;
    for(int i = 0; i < 256; i++)
    {
        var = i;
        for(int j = 0; j < 32; j++)
        {
            if(var & CRC32_MSB_CHECK)
            {
                var <<= 1;
                var ^= CRC32_MPEG_GENERATOR;
            }
            else
            {
                var <<= 1;
    ...
    for(int i = 0; i < 256; i++)
    {
        if(i%16 == 0)
            printf("\n");
        printf("0x%08x, ", crc32_tab[i]);
    }
    return;
}


void crc8_table_make()
{
    uint8 crc8_tab[256] = { 0 };
    uint8 var = 0;
    for(int i = 0; i < 256; i++)
    {
        var = i;
...
uint8 crc8(uint8 init, uint8* pdata, uint32 bytes)
{
    for(uint32 i = 0; i < bytes; i++, pdata++)
    {
        init = (init << 8) ^ crc8_tab[(init ^ *pdata) & 0xFF];
    }
    return init;
}

uint32 crc32(uint32 init, uint8* pdata, uint32 bytes)
{
    for(uint32 i = 0; i < bytes; i++, pdata++)
    {
        init = (init << 8) ^ crc32_tab[((init >> 24) ^ *pdata) & 0xFF];
    }
    return init;
}
int main()
{
    //crc8_table_make();
    // crc32_table_maker();
    uint8  pdata[3] = {0x03, 0x04, 0x07};
    // uint8 crc8val = crc8(0, pdata, 3);
    // printf("crc8val = %d====%x\n", crc8val, crc8val);

    uint32 crc32val = crc32(0xFFFFFFFF, pdata, 3);
    printf("crc32val = %d====%x\n", crc32val, crc32val);
    return 0;
}